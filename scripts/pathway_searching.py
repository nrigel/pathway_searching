    ...
    def submotifmatcher(self, C, H):
        with NMRT._driver.session() as db:
            ...
            for i in range(self.r[0], self.r[1]):
                ...
                'THEN CASE', 'WHEN (abs(Cexp-toFloat(split(s.CS, " ")[2]))/toFloat(split(s.CS, " ")[2])+abs(Hexp-toFloat(split(s.CS, " ")[4]))/toFloat(split(s.CS, " ")[4]))/2 <= cutoff',
                'THEN [s.SMILES, (abs(Cexp-toFloat(split(s.CS, " ")[2]))/toFloat(split(s.CS, " ")[2])+abs(Hexp-toFloat(split(s.CS, " ")[4]))/toFloat(split(s.CS, " ")[4]))/2] END',
                'WHEN toInteger(split(s.CS, " ")[2]) = 0 AND toInteger(split(s.CS, " ")[4]) > 0',
                'THEN CASE', 'WHEN (abs(Cexp-Cavg)/Cavg+abs(Hexp-toFloat(split(s.CS, " ")[4]))/toFloat(split(s.CS, " ")[4]))/2 <= cutoff',
                'THEN [s.SMILES, (abs(Cexp-Cavg)/Cavg+abs(Hexp-toFloat(split(s.CS, " ")[4]))/toFloat(split(s.CS, " ")[4]))/2] END',
                ...
                'END', 'ELSE CASE', 'WHEN (abs(Cexp-Cavg)/Cavg+abs(Hexp-Havg)/Havg)/2 <= cutoff',
                'THEN [s.SMILES, (abs(Cexp-Cavg)/Cavg+abs(Hexp-Havg)/Havg)/2] END END AS result'])

                matches = {m[0][0]: m[0][1] for m in db.run(a).values() if m[0] != None}
                print(matches, '\n')
