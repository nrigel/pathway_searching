...
def Draw(SMILES, molSize=(450,150)):
...
with open(args[1], 'rb') as pkl:
    smiles = pickle.load(pkl)

SVGs = {m: Draw(smiles[m]) for m in smiles}
SVGs = {m: SVGs[m] for m in SVGs if SVGs[m]}

with open(args[1][:-4]+'_SVGs.pkl', 'wb') as pkl:
    pickle.dump(SVGs, pkl, pickle.HIGHEST_PROTOCOL)
